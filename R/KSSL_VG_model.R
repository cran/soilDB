...
KSSL_VG_model <- function(VG_params, phi_min=10^-6, phi_max=10^8, pts=100) {
  ...
  m$theta <- .vg(phi= m$h, theta_r = VG_params$theta_r, theta_s = VG_params$theta_s, alpha = 10^(VG_params$alpha), n = 10^(VG_params$npar))
  
  # use splines to fit inverse model
  # scale of theta {0,1}
  # phi: units of kPa
  vg.inv <- splinefun(m$theta, m$phi)
  
  # return curve and spline function
  return(list(VG_curve=m, VG_inverse_function=vg.inv))
}
