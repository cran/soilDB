...
test_that("fetchSDA() returns expected results", {
  ...
  expect_equivalent(x$nationalmusym, c('kzc4', 'kzc4'))
  expect_equivalent(aqp::site(x)$nationalmusym, c('kzc4', 'kzc4'))
...
test_that("fetchSDA(duplicates=TRUE) works as expected", {
  ...
  expect_equivalent(length(x2) * length(unique(x3$mukey)), length(x3))
})
