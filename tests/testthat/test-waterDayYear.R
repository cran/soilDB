...
test_that("works as expected", {
  ...
  # check years
  expect_equivalent(res.lr$wy, 2008L)
  expect_equivalent(res$wy, 2009L)

  # days are offset by 1
  expect_equivalent(res.lr$wd, 153L)
  expect_equivalent(res$wd, 152L)

  # first day of water year 2019
  x <- as.Date("2018-10-01")
  res <- waterDayYear(x)
  
  expect_equivalent(res$wy, 2019)
  expect_equivalent(res$wd, 1)

  # last day of water year 2018
  x <- as.Date("2018-09-30")
  res <- waterDayYear(x)

  expect_equivalent(res$wy, 2018)
  expect_equivalent(res$wd, 365)
  ...
  expect_equivalent(res$wy, c(1983, 1981))
  expect_equivalent(res$wd, c(50,  140))
  ...
  expect_equivalent(res$wy, 2001L)

  expect_equivalent(res$wd, 66L)
...
test_that("non-standard `end` of water year", {
  ...
  expect_equivalent(res$wy, 2019L)
  expect_equivalent(res$wd, as.integer(round(difftime("2018-09-30", "2018-03-10"))))
})
